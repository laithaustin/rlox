...
fn test_block_scoping_inner_variable() {
    ...
    assert!(result.is_err(), "Expected runtime error for undefined variable");
    
    // Should have a runtime error for undefined variable y
    assert!(!reporter.runtime_errors.is_empty(), "Expected runtime error to be reported");
    
    // Verify that the error mentions 'y' being undefined
    let error_msg = &reporter.runtime_errors[0];
    assert!(error_msg.contains("Undefined variable 'y'"), 
            "Error should mention y is undefined, but got: {}", error_msg);
}
...
fn test_block_reassign_outer_from_inner() {
    ...
    assert!(result.is_ok(), "Should be able to access variables from outer scope");
    reporter.assert_no_errors();
    reporter.assert_no_runtime_errors();
...
fn test_block_syntax_error() {
    ...
    if result.is_err() {
        ...
        assert!(error.message.contains("'}'") || error.message.contains("block") || error.message.contains("Expected"),
                "Error should mention missing block closing, but got: {}", error.message);
    } else {
        ...
        assert!(error_msg.contains("'}'") || error_msg.contains("block") || error_msg.contains("Expected"),
                "Error should mention missing block closing, but got: {}", error_msg);
    }
}
